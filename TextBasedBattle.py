from Dialogue import intro_text, intro_text2, intro_text3, intro_text4, tutorial_text, space, line
from Dialogue import total_items, total_available_actions, body_check_opener, format_objects_string, format_input
from Dialogue import blunt_text, Soundcloud_text1, Soundcloud_text2, G_text1, G_text2, poppers_text, cigarette_text, chewing_gum_text, lollipop_text
import random
...
class Battle:
	...
	def update_items():  #It calls the active item function inside.
		...
		player.active_items = updated_items #Here we have an updated list after it's over.
	...
	def update_room(): #Updates people in this room, informs where you are and what you can do. Calls choose_action in the end.
		...
		available_actions_string = format_objects_string(player.location.available_actions) #formats actions available
		...
		if len(usable_items_now) == 0:
			...
			print ("In this room, you could probably safely [USE] your " + usable_items_now_string)
	...
	def cool_enough(interactionee): #Checks your coolness against a person's how_cool attr
	...
	def identify_person(person_to_identify): #takes input string, returns a person object, returns None if else

			if "part" in person_to_identify:
				return partner
			
			elif "smile" in person_to_identify or "embass" in person_to_identify or "ruben" in person_to_identify:
				return smile
			
			elif "russ" in person_to_identify or "friend" in person_to_identify or "misha" in person_to_identify:
				return misha
			
			elif "cut" in person_to_identify or "tank" in person_to_identify or "top" in person_to_identify:
				return tanktop
	...
	def identify_item(item_to_identify): #takes input string, returns an item string, returns None if else
			...
			elif "blunt" in item_to_identify or "weed" in item_to_identify or "hash" in item_to_identify:
				return "blunt"
			
			elif item_to_identify == "g" or "ghb" in item_to_identify or "chorr" in item_to_identify:
				return "G"
	...
	def move():   #The action of moving. This must be set up for every new room in choose_room.
		...
		def not_there(this_room): #checks if you're in the room, if you aren't moves you there
	...
	def tutorial():  #Tutorial as action in the game
		...
		action = None
		while action == None:
			print ("Need to [REMEMBER] what you can do or ready to choose an [ACTION]?", end = " ")
			action = input ()
	...
	def borrow(): #borrow an item from a person 
		def in_this_room(borrowee): #takes a person object returns True if in current room
		...
		while action == None:
			...
			if action == "no" or action == "nobody" or action == "no one" or action == "noone":
			...
			if person == None:
			...
			elif Battle.cool_enough (person) == False:  #checks if youre cool enough to borrow from them
				...
				if format_objects_string(person.items) == "nothing":
					...
					Battle.choose_action()
					break
				print ("They have " + (format_objects_string(person.items)[:-1]) + " on them.")
				...
				while item_borrowed == None:
					...
					if item_borrowed == "nothing" or action == "no":
						print ("You change your mind and politely decide not to borrow anything.")
						Battle.choose_action()
						break
	...
	def choose_action(): #Prompts the player to the desired action
		def doable_in_room(doable_action): #To see if you can do an action in this room
...
def yesorno(): #You should try to make it work with yesornoresult[0] with an index error exception
    yesornoresult = None
    while yesornoresult == None:
        yesornoresult = input()
        yesornoresult = yesornoresult.lower()
        if "y" in yesornoresult:
            return True
        if "n" in yesornoresult:    
            return False
        else:
            print ("(yes or no):", end = " ")
            yesornoresult = None
...
class Player:
	def __init__(self, name, lit, high, coolness, items, active_items, location, memories = [], gameover=False):
		self.name = name
		self.lit = lit
		self.high = high
	...
	def use_item(self, item_used):  ###Used to call battle.update items(), changed it to time sensitive
		...
		if item_used in self.items:
			...
			if item_used == "lollipop" or item_used == "cigarette" or item_used == "chewing gum":
...
class NPC(Player):
	def __init__(self, name, items, how_cool, location, times_talked = 0, flirt = None): ###Items are passed in a LIST
		self.name = name
		self.items = items
...
class Party:
	...
	def __repr__(self):
		return "The party is currently {crowded}. The music is {music}".format(crowded=self.crowded, music = self.music)
...
total_items= {"lollipop": 50, "Soundcloud":70, "G": 40, "blunt": 40, "cigarette":20, "poppers": 20, "chewing gum": 40}
### Item Setup above
...
smoking_room = Room ("the smoking room", ["[FLIRT]", "[BORROW] something", "[MOVE] to another room", "[LEAVE] the party"], ["lollipop", "Soundcloud", "blunt", "cigarette", "chewing gum"])
bathroom = Room ("the bathroom", ["[FLIRT]", "drink [TAP] water", "[MOVE] to another room", "[LEAVE] the party"], ["lollipop", "Soundcloud", "G", "poppers", "chewing gum"])
dance_floor = Room ("the dance floor", ["[FLIRT]", "[DANCE]", "[BORROW] something", "[MOVE] to another room", "[LEAVE] the party"], ["lollipop", "Soundcloud", "poppers", "chewing gum"])
...
player = Player("Joel", 20, 0, 10, {"lollipop": 50, "Soundcloud":70, "blunt": 40, "poppers": 20, "chewing gum": 40}, {}, smoking_room)
...
tanktop = NPC("the cutie in the tank top", ["Soundcloud", "blunt", "cigarette", "lollipop"], 50, dance_floor)
pusher = NPC("someone who seems to be a pusher", ["Soundcloud", "blunt", "cigarette", "poppers", "chewing gum"], 90, bathroom)
hottest = NPC("a breathtaking beauty", ["lollipop", "cigarette"], 90, smoking_room)
couple = NPC("an attractive couple", ["blunt", "cigarette", "chewing gum"], 20, dance_floor)
...
total_available_actions= ["[FLIRT]", "[DANCE]", "[TALK]", "[BORROW] something", "drink [TAP] water", "[MOVE] to another room", "[LEAVE] the party", "lollipop", "Soundcloud", "G", "blunt", "cigarette", "poppers", "chewing gum"]
time = Time(1, 30)
...
def smileborrowGcode ():
	...
	""")
			...
			print ("You sure you wanna go all the way to the bathroom for this? (y/n)", end = " ")
			option = yesorno()
			print ("")
			if option == True:
				...
				if party.crowded == "crowded":
					print ("The smile ambassador and you slowly bump your way the crowd around to the bathroom.")
					time.ten_minutes()
				if party.crowded != "crowded":
	...
	""")
		...
		if player.location == bathroom:  
			if smile.times_talked == 3: #This is if you havent met the pusher yet, similar to scene 3.
				print ("""
...
################################### SmileTimes3
smileborrowG.run_scene()
#NOTE Still have to check the borrowGscene
...
time.ten_minutes()
time.ten_minutes()
party.crowded = "crowded"
player.location = dance_floor
smileborrowG.run_scene()
player.location = smoking_room
smileborrowG.run_scene()
##This scene isnt accepting the no option for some reason, 
##think its because of tabbing the option disappears from the memory, domain stuff