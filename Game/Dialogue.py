...
Soundcloud_text2= "You can't help your body moving to the music. Soundcloud's coming up."
...
def format_objects_string(given_list):#Returns "nothing" if empty list, returns "object, object, and object." style string. 
	final_string = ""
	...
	try:
		if hasattr(given_list[0], "name") == True: #If it's a list of objects
			...
			if len(given_list) >= 3:
				...
				final_string += given_list[-2].name + " and " + given_list[-1].name + "."
			return final_string
	except KeyError:
		pass
...
total_available_actions= ["leave", "go somewhere else", "flirt", "move to the music", "dance", "borrow cigarette", "drink tap water"]
